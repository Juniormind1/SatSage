...
- **Anmeldung · Passwort vergessen:** Wer das SatSage-Passwort nicht mehr weiß, kann die verschlüsselte Konfiguration verwerfen. Der Folgedialog sagt, dass Wallets, Knoten und `.env` danach von vorn eingerichtet werden. „Von vorn anfangen“ leert die Datei, „Ich denk nochmal nach“ lässt sie stehen.
- **StartOS · kein eigenes Passwort:** StartOS erzeugt kein `uiPassword` mehr, schickt keins in die App und zeigt keine Actions zum Anzeigen oder Rotieren. Benutzer `admin` gibt es dort nicht. Ein Hash aus einem älteren Paket wird beim Start gelöscht, wenn die `.env` nicht scrambled ist. Die Oberfläche öffnet ohne Token. Ein Passwort gibt es nur, wenn du es in den SatSage-Einstellungen setzt.
- **StartOS · Oberfläche ohne Token:** Liegt die `.env` im Klartext, gibt es kein Passwort und kein `?t=` in der Web-UI. Die API gilt dann als angemeldet. „Token fehlt“ erscheint dort nicht mehr. Ist die `.env` scrambled, bleibt die Anmeldung das App-Passwort.
- **StartOS · Passwort nur mit verschlüsselter .env:** Ein von StartOS erdachtes `uiPassword` ist kein SatSage-Passwort, solange die `.env` im Klartext liegt. Dann bleibt die Oberfläche offen, ein liegengebliebener Hash zählt nicht.
- **StartOS · Browser:** Das Basic-Auth-Gate am Proxy ist weg. Safari, Firefox und Chrome bekommen dieselbe App-Anmeldung (Benutzer `admin`, Passwort über *Show Web UI Password*). Die `127.0.0.1`-Zeile im Log ist die Adresse im Container und kein Link für den Client.
- **StartOS · Admin-Passwort:** *Show Web UI Password* zeigt das aktuelle Basic-Auth-Passwort (Benutzer `admin`) maskiert und kopierbar, auch bei laufendem Dienst. *Rotate Web UI Password* erzeugt ein neues, ohne SatSage zu stoppen; Basic Auth und der Login-Hash gelten sofort. Das ist das StartOS-Admin-Passwort, nicht das Desktop-App-Passwort und nicht der Schlüssel der `.env`-Verschlüsselung.
- **StartOS · API nach Proxy-Login:** Ein leerer `X-Satsage-Token` (Firefox ohne sessionStorage, Safari mit Restzustand) gilt nicht mehr als Fehl-Token. Hinter dem vertrauten StartOS-Proxy geht die API ohne Token und ohne Sitzungsspeicher durch. Der Client schickt den Header nur noch mit, wenn wirklich ein Token da ist.

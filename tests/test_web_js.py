...
class TestAppJs(unittest.TestCase):
    ...
    def test_chrome_scripts_stehen_vor_boot(self):
        ...
        html = (WEB / "index.html").read_text(encoding="utf-8")
        pos_api = html.index('<script src="/api.js')
        pos_state = html.index('<script src="/state.js')
        pos_format = html.index('<script src="/format.js')
        pos_app = html.index('<script src="/app.js')
        pos_mempool = html.index('<script src="/mempool_links.js')
        pos_nav = html.index('<script src="/chrome_nav.js')
        pos_chrome = html.index('<script src="/chrome.js')
        pos_boot = html.index('<script src="/boot.js')

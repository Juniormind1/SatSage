...
(() => {
  ...
  const MUSTER = [
    ...
    [/^(.+): scantxoutset ab Index #(\d+) \((\d+) weitere je Kette, normal bis #(\d+)\)…$/,
      "$1: scantxoutset from index #$2 ($3 more per chain, normal scan only to #$4)…"],
    [/^Schatz: (\d+) sats auf Empfang #(\d+) von (.+)\. Der normale Scan reicht nur bis Index #(\d+) — diese Adresse lag außerhalb des Suchfensters\.$/,
      "Treasure: $1 sats on receive #$2 of $3. The normal scan only reaches index #$4 — this address was outside the search window."],
    [/^Schatz: (\d+) sats auf Wechselgeld #(\d+) von (.+)\. Der normale Scan reicht nur bis Index #(\d+) — diese Adresse lag außerhalb des Suchfensters\.$/,
      "Treasure: $1 sats on change #$2 of $3. The normal scan only reaches index #$4 — this address was outside the search window."],
    [/^(\d+) UTXOs außerhalb des Suchfensters\.$/, "$1 UTXOs outside the search window."],
    [/^(\d+) UTXO außerhalb des Suchfensters\.$/, "$1 UTXO outside the search window."],
    [/^Kein UTXO außerhalb des Suchfensters\.$/, "No UTXO outside the search window."],
    [/^Keine Wallets — nichts zu suchen\.$/, "No wallets — nothing to search."],

...
(() => {
  ...
  const MUSTER = [
    ...
    [/^Kurs: aktueller Kurs nicht beschaffbar, letzter Kurs aus Historie von (.+) wird verwendet\.$/,
      "Price: current rate unavailable, using last history rate from $1."],
    [/^Kurs: aktueller Kurs nicht beschaffbar, Tageskurs aus Historie von (.+) wird verwendet\.$/,
      "Price: current rate unavailable, using history day rate from $1."],
    [/^Kurs: aktueller Kurs nicht beschaffbar\.$/,
      "Price: current rate unavailable."],
    ...
    [/^FiFo-Kandidaten: fehlgeschlagen — (.+)$/,
      "FiFo candidates failed — $1"],
    // Legacy-Zeilen derselben Session
    [/^Selbstanzeige: Kandidaten aus Cache…$/,
      "FiFo candidates: from cache…"],
    [/^Selbstanzeige: (\d+) Abfluss-Kandidat\(en\) · (\d+) UTXO\(s\) Was-wäre-wenn\.$/,
      "FiFo candidates: $1 outflow(s) · $2 what-if UTXO(s)."],
    [/^Selbstanzeige: Kandidaten fehlgeschlagen — (.+)$/,
      "FiFo candidates failed — $1"],
    ...
    [/^Tor-SOCKS (.+) wieder erreichbar \(in \.env: (.+)\)$/,
      "Tor SOCKS $1 reachable again (in .env: $2)"],
    [/^Tor-SOCKS (.+) wieder erreichbar$/, "Tor SOCKS $1 reachable again"],
    ...
    [/^Lösche (.+)$/, "Deleting $1"],
    [/^Lösche (.+): Datei (\d+)\/(\d+)…$/, "Deleting $1: file $2/$3…"],
    [/^Löschen beendet$/, "Delete finished"],
    [/^Adressen nachziehen braucht Indexer \(kein Electrs\/Fulcrum konfiguriert\)\.$/,
      "Fill addresses needs indexer (no Electrs/Fulcrum configured)."],
    [/^Adressen nachziehen braucht Indexer — noch nicht verbunden \(z\. B\. Tor startet noch\)\. Warte…$/,
      "Fill addresses needs indexer — not connected yet (e.g. Tor still starting). Waiting…"],
    [/^Adressen nachziehen braucht Indexer — warte weiter \(noch ~(\d+)s\)…$/,
      "Fill addresses needs indexer — still waiting (~$1s left)…"],
    [/^Adressen nachziehen braucht Indexer \(Timeout — Tor\/Electrs nicht erreichbar\)\. Später erneut oder „Historie“\.$/,
      "Fill addresses needs indexer (timeout — Tor/Electrs unreachable). Retry later or use “History”."],
    [/^Indexer verbunden — Adressen nachziehen…$/, "Indexer connected — filling addresses…"],
    [/^Adressen nachziehen für (.+): (\d+) Tx ohne Adresse…$/,
      "Fill addresses for $1: $2 txs without address…"],
